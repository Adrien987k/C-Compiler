...
Le mot clé _let_ permet de déclarer une association entre un nom et une expression d'un certain type. Si ce type ne comporte pas le mot clé _ref_, cette association peut être vu comme un élément de sucre syntaxique permettant d'écrire des programmes plus concis. 
...
Pour le _CRETURN_ : Si lors de l’exécution du premier bloc d'un _try_, si l'on tombe sur un _return_ et si la clause _try_ comporte un bloc _finally_, ce dernier bloc doit absolument être exécuter avant le retour de la fonction. 
...
* catch\_label : Lors de la compilation du code d'un bloc _try_, on génère un label assembleur vers lequel sauter pour arriver devant un code qui effectuera les comparaisons entre l'exception levée et les différentes exceptions potentiellement attrapées. Ce label est stocké dans la variable _catch\_label_.
...
* finally\_label : Lors de la compilation d'un _try_, un label assembleur est généré et correspond à l'endroit où sauter afin d'aller à l’exécution de la clause _finally_
...
* Compilation du CTRY :
    ...
    * S'il y a un bloc _fianlly_, on génère un label permettant de sauter au _finally_
